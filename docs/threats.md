...
|---|---|---|---|---|
| **Serial GPS USB disconnect** (cable jiggle, EMI, kernel renumeration) | Reconnect-with-exponential-backoff loop inside `SerialGpsSource::into_stream`: 250 ms initial, doubling up to 5 s. Logs disconnect + reconnect at warn level | `src/hw/serial_gps.rs:97-150` | Visual; live exercise on real hardware recommended | If the OS doesn't release the FD promptly on disconnect (kernel-driver bug), reconnect may EBUSY-loop for ~1-2 s until renumeration completes |
| **I²C IMU bus down** (chip gone, kernel I2C controller error) | Consecutive-error counter: 100 errors → `warn!` "DEGRADED" once; 1000 errors → end the stream so systemd restarts the process. Single error transients ignored; recovery (successful read) logs `info!` "RECOVERED" | `src/hw/i2c_imu.rs:108-200` | Counter-based, no unit test; verified by code review (`AUDIT E-14` comment) | Between degraded threshold (1 s of failures at 100 Hz) and bus-down threshold (10 s), the IMU is silently absent but flag is set. Operator must watch logs |
...
| **Atomic write** — process crash mid-dump must NOT leave partial JSON on disk | `.tmp` + rename pattern; `O_EXCL` on create; `0o600` mode on Unix | `src/forensic.rs:312-375` | `dump_writes_atomic_file_and_returns_path` |
...
- **Time-source attacks.** All timestamps use `tokio::time::Instant`
  (monotonic, immune to wall-clock manipulation). The JSON event log
