...
| **PX4 vs ArduCopter mode confusion** — same `custom_mode` integer means different things; flat (`6=RTL`) for ArduCopter vs packed `(sub<<24)|(main<<16)` for PX4 | `is_rtl_mode(profile, custom_mode)` dispatches per-profile. PX4 path checks `main=AUTO=4` and `sub ∈ {RTL=5, LAND=6, RTGS=7, DESCEND=20}` (DESCEND is PX4's position-less emergency descent — landing-class, observed live as `0x14040000` after the GPS sever); ArduCopter flat enum (6/9/21) | `src/mav/monitor.rs:90-114` | `rtl_mode_recognition_arducopter`, `rtl_mode_recognition_px4`, `px4_mode_encoding_round_trips` | None for these two profiles. Plane / Rover / other profiles would need their own dispatch arm |
